...
const categories = [
  { name: "도형", values: Shapes },
  { name: "카테고리1", values: cateTest1 },
  { name: "카테고리2", values: cateTest2 },
];
...
function shapes() {
  ...
  const content = document.querySelector(".toolpanel#shapes-panel .content");
  ...
  CategoryList.forEach((category) => {
    ...
    shapes.classList.add("draw-shapes");
    categoryArea.appendChild(shapes);
    values.forEach((svg) => {
    ...
    categoryList.appendChild(categoryArea);
  });
  content.appendChild(categoryList);

  // 날씨 기호 카테고리 추가
  const weatherIconList = document.createElement("div");
  weatherIconList.classList.add("category-area");

  const weatherIconListTitle = document.createElement("h4");
  weatherIconListTitle.classList.add("category-title");
  weatherIconListTitle.textContent = "날씨기호";
  weatherIconList.appendChild(weatherIconListTitle);

  const weatherIconsContainer = document.createElement("div");
  weatherIconsContainer.classList.add("draw-shapes");
  weatherIconList.appendChild(weatherIconsContainer);

  // JSON 파일에서 날씨 기호 불러오기
  fetch("/json/icons.json") // JSON 파일 경로
    .then((response) => response.json())
    .then((data) => {
      data["날씨 기호"].forEach((icon) => {
        const button = document.createElement("div");
        button.classList.add("button");
        ...
        button.addEventListener("click", function () {
          try {
            fabric.Image.fromURL(icon.src, (imgObj) => {
        ...
        weatherIconsContainer.appendChild(button);
      });
    })
    .catch((error) => {
  ...
  categoryList.appendChild(weatherIconList);
}
