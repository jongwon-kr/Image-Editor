...
function copyPaste(canvas) {
  ...
  document.addEventListener("keydown", async (e) => {
    ...
    if (e.key === "c" || e.key === "C") {
      ...
      if (!activeObject) {
        console.log("복사할 객체가 선택되지 않았습니다.");
        return;
      }
    ...
    if (e.key === "x" || e.key === "X") {
      ...
      const activeObject = canvas.getActiveObject();
      if (!activeObject) {
        console.log("잘라낼 객체가 선택되지 않았습니다.");
        return;
      }
      ...
      try {
        if (activeObject.type === "image") {
          ...
          canvas.renderAll();
          console.log("이미지 잘라냄 (클립보드):", dataUrl);
        } else {
          activeObject.clone(async (cloned) => {
            const jsonData = JSON.stringify(cloned.toJSON());
            await navigator.clipboard.writeText(jsonData);
            ...
            canvas.discardActiveObject();
            canvas.renderAll();
            console.log("객체 잘라냄 (클립보드):", jsonData);
          });
        }
    ...
    if (e.key === "v" || e.key === "V") {
      ...
      try {
        ...
        if (textData) {
          ...
          if (isJSONObjectString(textData)) {
            ...
            if (!validTypes.includes(obj.type)) {
              console.log("지원되지 않는 객체 타입:", obj.type);
              return;
            }
        ...
        console.log("붙여넣을 유효한 데이터가 클립보드에 없습니다.");
      } catch (err) {
